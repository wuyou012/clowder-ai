...
/**
 * F253 Phase C — qc-metrics-rollup sourceRefs. Replayable rollup window selector:
 * provider resolves to a QcMetricsSnapshot (finding yield, false positive rate,
 * reviewer delta, post-merge bug rate). Phase C bootstrap returns zero-baseline;
 * live data sources wired in future phases.
 *
...
const qcMetricsSourceRefsShape = z
  .object({
    ...
    windowStartMs: z
      ...
      .finite()
      .describe(
        'Inclusive epoch ms window start for QC metrics rollup (finding yield, FP rate, reviewer delta, post-merge bug rate).',
      ),
    windowEndMs: z
      .number()
      .finite()
  ...
  .describe('eval:qc sourceRefs — replayable QC metrics rollup window selector (F253 Phase C).');
