...
describe('tryCliProbe (unit)', () => {
  ...
  test('codex probe closes stdin so exec does not wait for piped input', async () => {
    const { tryCliProbe } = await import('../dist/routes/first-run-quest.js');
    let capturedCmd = '';
    const exec = async (cmd) => {
      capturedCmd = cmd;
    ...
    const result = await tryCliProbe('codex', { model: 'gpt-5.5', execFn: exec });
    assert.ok(result);
    assert.equal(result.ok, true);
    assert.ok(capturedCmd.includes('--model gpt-5.5'));
    assert.ok(capturedCmd.includes(process.platform === 'win32' ? '< NUL' : '</dev/null'));
  });
  ...
  test('forwards env vars to CLI subprocess', async () => {
    ...
    assert.equal(capturedOpts.env[pathKey], process.env[pathKey]);
  });
