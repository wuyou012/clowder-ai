...
> **Implementation**: clowder-ai#619 intake (AC-G1..G8 + G9 LocalTraceStore TTL, merged 2026-05-08); Phase G-Followup native L0 closure (AC-G10 / KD-43, merged 2026-06-03 closing the KD-42 gap — `PromptCapture` now persists `nativeSystemPrompt` for F203 native-L0 providers + Hub X-Ray Inspector System tab shows Native L0 zone above message-system pack appendix).
...
- [x] AC-G10: Prompt X-Ray 必须覆盖 F203 native L0 channel，或在 UI/API 中明确拆分 native system prompt vs user/effective prompt，避免把空 `System` tab 误读成未捕获. Closed in Phase G-Followup (2026-06-03, KD-43): `PromptCapture` schema gained `nativeSystemPrompt` / `nativeSystemPromptSource` / `nativeSystemTokenEstimate` / `totalTokenEstimate` / `captureDiagnostics`; bridge async-fetches L0 from `compileL0ViaSubprocess` for `service.injectsL0Natively()` providers (Claude/Codex); Hub X-Ray Inspector System tab now zones Native L0 (system role) above the message-system pack appendix; resume amber banner now distinguishes message-system path from native L0; token bar and Meta tab break out native L0 vs message tokens to fix silent under-count. `l0-compiler.ts` gained in-flight Promise dedup so capture and native injection sharing a cold cache don't double-spawn. Test coverage: `l0-compiler.test.js` 14/14 (3 new AC-G10 dedup tests) + `prompt-capture.test.js` 20/20 (5 new AC-G10 tests: backward compat, native capture, fail-safe diagnostic, non-native passthrough).
...
| KD-43 | Phase G AC-G10 (KD-42 closure): `PromptCapture` 持久化 `nativeSystemPrompt` data field 直接拿 `compileL0ViaSubprocess` 编译后 L0，Hub UI System tab 分区显示 Native L0 / message-system pack，token 估算分桶（msg / native L0 / total）。Capture 走 fire-and-forget async runner，L0 fetch 失败只写 `captureDiagnostics`，不阻塞 invocation hot path。`l0-compiler.ts` 加 in-flight Promise dedup，capture + native injection 并发冷启动只发一次 subprocess | 砚砚 Design Gate（2026-06-03）四立场全部 actionable：(1) 不要重复编译 → 加 in-flight dedup + capture 异步取 + fail-safe diagnostic；(2) 保留 `systemPrompt` 不 rename → 加 `nativeSystemPrompt` 双字段，避免 UI 误读 non-native provider 的 system；(3) 单 System tab 分区，Native L0 上方 + pack appendix 下方，修正 resume banner 误导（injected=false 只代表 message-system path 不影响 native L0）；(4) 保留 `tokenEstimate = effectivePrompt`，新增 `nativeSystemTokenEstimate` + `totalTokenEstimate` Hub 顶部显示 total / Meta 拆开 | 2026-06-03 |
